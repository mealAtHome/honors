...
GGC.Common =
{
    ...
    priceWonFont (val) { return `<span class="common-font" font-color="${val >= 0 ? 'pstv' : 'ngtv'}">${GGC.Common.comma(val)}원</span>`; },
    ...
    wonColor(val) { return `<span class="common-font" font-color="${val >= 0 ? 'pstv' : 'ngtv'}">${GGC.Common.optpriceWon(val)}</span>`; }, /* Mark + Price + Won + Font */
